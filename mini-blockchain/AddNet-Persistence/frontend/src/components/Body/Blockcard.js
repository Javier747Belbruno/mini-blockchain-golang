...
function Blockcard({ reload }) {
  ...
  const FetchBlocks = async () => {
    setLoading(true);
    await axios
      .get(apiBaseUrl + "/api/getblockchain")
      .then((res) => {
        setdata(res?.data);
        setLoading(false);
      })
  ...
  return (
    <div className="flex flex-col align-center justify-center p-6">
      ...
      {!loading && (
        <div>
          <h1 className="dark:text-white text-2xl mb-5">
            Distributed Ledger, the only source of truth.
          </h1>
        </div>
      )}
        ...
        <div className="grid-cols-1 grid sm:grid-cols-4 grid-flow-row-dense gap-4 ">
          {data?.map((bc, i) => (
            ...
            >
              ...
              {bc.transactions.map((item, i) => (
                <div
                  className="p-4 border-[1px] border-[#3d4f7c] rounded-md hover:bg-slate-50 dark:text-white dark:hover:bg-slate-800"
                  key={i}
                >
                  <p className="break-all">Hash: {item.hash}</p>
                  <p className="break-all">Timestamp: {item.timestamp}</p>
                  <p className="break-all">From: {item.from}</p>
                  <p className="break-all">To: {item.to}</p>
                  <p className="break-all">Amount: {item.amount}</p>
                </div>
              ))}
            </div>
          ))}
