...
function Body() {
  ...
  const NewTx = async () => {
    if (sender === "" && receiver === "" && amount === "") {
      ...
      await axios
        .post(apiBaseUrl + "/api/sendtransaction", postMe)
        .then((res) => {
      ...
      <div className="flex flex-col flex-1 items-center justify-start w-full mf:mt-0 mt-10">
        ...
        <div className="p-5 sm:w-96 w-full flex flex-col justify-start items-center blue-glassmorphism bg-slate-50 dark:bg-slate-900 shadow-sm">
          <input
            ...
            placeholder="what's your name? Ezekiel?"
            type="text"
          ...
          <input
            ...
            placeholder="what's your friend's name? Tony?"
            type="text"
            className="my-2 w-full rounded-md p-2 outline-none bg-transparent dark:text-white border-[1px] border-[#3d4f7c]  text-sm white-glassmorphism"
          />
          <input
            onChange={(e) => setAmount(e.target.value)}
            placeholder="How much do you wanna send? 10$ for coffee?"
            type="number"
            className="my-2 w-full rounded-md p-2 outline-none bg-transparent dark:text-white border-[1px] border-[#3d4f7c]  text-sm white-glassmorphism"
          />

          <h1 className="dark:text-white text-sm mt-4">
            Free blockchain, no gas fees
          </h1>
          <p className="text-gray-500 text-sm mt-4">
            {" "}
            Decentralized database and network all in one{" "}
          </p>
